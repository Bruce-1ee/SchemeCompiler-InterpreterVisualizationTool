...
(define (define-act-old m)
  (cond
    ((eq? m 'self-evaluating)
     (let ((org-eval-self-evaluating eval-self-evaluating)
           (org-compile-self-evaluating compile-self-evaluating))
       (set! eval-self-evaluating
             (lambda (exp)
               (output "constant " exp " -- exec")
               (org-eval-self-evaluating exp)))
       (set! compile-self-evaluating
             (lambda (exp next)
               (list 'act-constant (org-compile-self-evaluating exp next))))))
    ((eq? m 'quotation)
     (let ((org-eval-quotation eval-quotation)
           (org-compile-quoted compile-quoted))
       (set! eval-quotation
             (lambda (exp)
               (output "quotation " exp " -- exec")
               (org-eval-quotation exp)))
       (set! compile-quoted
             (lambda (exp next) (list 'act-constant (org-compile-quoted exp next))))))
    ((eq? m 'variable)
     (let ((org-eval-variable eval-variable)
           (org-compile-variable-local compile-variable-local)
           (org-compile-varable-free compile-varable-free))
       (set! eval-variable
             (lambda (exp env)
               (output "variable " exp " -- exec")
               (org-eval-variable exp env)))
       (set! compile-variable-local
             (lambda (n m next) (list 'act-refer (org-compile-variable-local n m next))))
       (set! compile-varable-free
             (lambda (var next) (list 'act-refer (org-compile-varable-free var next))))))
    ((eq? m 'assignment)
     (let ((org-eval-assignment eval-assignment))
       (set! eval-assignment
             (lambda (exp env)
               (output "assignment " exp " -- exec")
               (org-eval-assignment exp env)))))
    ((eq? m 'definition)
     (let ((org-eval-definition eval-definition))
       (set! eval-definition
             (lambda (exp env)
               (output "definition " exp " -- exec")
               (org-eval-definition)))))
    ((eq? m 'if)
     (let ((org-eval-if eval-if)
           (org-eval-if-test eval-if-test)
           (org-eval-if-then eval-if-then)
           (org-eval-if-else eval-if-else)
           (org-compile-if-then compile-if-then)
           (org-compile-if-else compile-if-else)
           (org-compile-if-test compile-if-test))
       (set! eval-if
             (lambda (exp env)
               (output "if" "" " -- exec")
               (org-eval-if exp env)))
       (set! eval-if-test
             (lambda (test env)
               (output "test" "" " -- exec")
               (org-eval-if-test test env)))
       (set! eval-if-then
             (lambda (then env)
               (output "then" "" " -- exec")
               (org-eval-if-then then env)))
       (set! eval-if-else
             (lambda (else env)
               (output "else" "" " -- exec")
               (org-eval-if-else else env)))
       (set! compile-if-then
             (lambda (then env next) (list 'act-then (org-compile-if-then then env next))))
       (set! compile-if-else
             (lambda (else env next) (list 'act-else (org-compile-if-else else env next))))
       (set! compile-if-test
             (lambda (test env next then else) (list 'act-if (list 'act-test (org-compile-if-test test env next then else)))))))
    ((eq? m 'lambda)
     (let ((org-eval-lambda eval-lambda)
           (org-compile-lambda compile-lambda))
       (set! eval-lambda
             (lambda (exp env)
               (output "lambda " exp " -- exec")
               (org-eval-lambda exp env)))
       (set! compile-lambda
             (lambda (exp env next) (list 'act-lambda (org-compile-lambda exp env next))))))
    ((eq? m 'application)
     (let ((org-eval-application eval-application)
           (org-eval-application-arg eval-application-arg)
           (org-eval-application-body eval-application-body)
           (org-compile-application-frame compile-application-frame)
           (org-compile-application-argument compile-application-argument)
           (org-compile-application-name compile-application-name))
       (set! eval-application
             (lambda (exp env)
               (output "function-call" "" " -- exec")
               (org-eval-application exp env)))
       (set! eval-application-arg
             (lambda (arg env)
               (output "argument" "" " -- exec")
               (org-eval-application-arg arg env)))
       (set! eval-application-body
             (lambda (name env)
               (output "body" "" " -- exec")
               (org-eval-application-body name env)))
       (set! compile-application-frame
             (lambda (next c) (list 'act-function-call (org-compile-application-frame next c))))
       (set! compile-application-argument
             (lambda (arg env c) (list 'act-argument (org-compile-application-argument arg env c))))
       (set! compile-application-name
             (lambda (exp env next) (list 'act-body (org-compile-application-name exp env next)))))))
    (set! VM-else
          (lambda (a x e s)
            (VM-else-with-breakpoint a x e s))))

(define all (list 'self-evaluating
                  'quotation
                  'variable
                  'assignment
                  'definition
...
(define (set-all-old lst)
  (if (null? lst)
      'done
      (begin
        (define-act-old (car lst))
        (set-all-old (cdr lst)))))
