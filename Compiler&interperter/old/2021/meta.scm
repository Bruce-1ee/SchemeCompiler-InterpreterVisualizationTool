        ...
        (if (not (pair? exp))
            (cond ((or (number? exp) (boolean? exp) (string? exp))
              ...
              (case (car exp)
                ...
                [else  ;exp: ( (lambda () 1) )
                 ;(if (not (and (symbol? (car exp)) (eq? 'primitive (car (car (lookup (car exp) env))))))
...
(define (exec-apply func arguments) ;函数执行
  ...
  (case (car func)
    ...
    ((function)  ;一般函数
     ...
     (display 'create_frame:)(display (caddr func)) (newline)
     (display 'env:) (display arguments) (newline)
...
;(meta  '( (lambda (a b) (+ a b)) 1 2))
;(meta '(define f (lambda(a) a)))
(meta '( (lambda (a b) ((lambda (c) (+ b c)) (+ a b))) 1 2))
