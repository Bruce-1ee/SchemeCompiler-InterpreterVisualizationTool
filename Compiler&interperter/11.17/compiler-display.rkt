...
;11.4 在find-sets, find-free函数中未添加begin相关代码
;11.18 已经向find-sets和find-free函数中添加了begin相关代码
         ...
         (case t ;;;;;;;;;
           ...
           [(set!)   ;;var (cadr x) x (caddr x)
            (set-union (if (set-member? (cadr x) v) (list (cadr x)) '())
                       (find-sets (caddr x) v))]
           [(begin)
            (letrec ((loop (lambda (exps ret)
                             (if (null? exps)
                                 ret
                                 (loop (cdr exps) (set-union ret (find-sets (car exps) v)))))))
              (loop (cdr x) '()))]
...
;;
;;寻找free变量，返回值为free变量的list
;;e.g. (find-free '(begin a b) '(b)) 返回值是a 
;;
(define find-free
  (lambda (x b)
    (cond
         ...
         (case t
           ...
           [(begin)
            (letrec ((loop (lambda(exps ret)
                             (if (null? (cdr exps))
                                 ret
                                 (loop (cdr exps) (set-union (find-free (car exps) b) ret))))))
              (loop (cdr x) '()))]
...
;(test '( (lambda(x) ( (lambda (a b) x ) 99 88) ) 77))
;(test '( (lambda(x) ( (lambda (a b) x ) 99 88) ) 77))
...
;(test '((lambda (a b c d) c) 11 22 33 44))
;(run '((lambda (a b c d) c) 11 22 33 44))

;(run '((lambda (a b) a) 1 ((lambda(a) a) 1)))

;(run '( (lambda(set a b)(set a b)) (lambda(a b)(set! a b)) 4 9))

;(run '(lambda(b)(lambda(a)(set! b 1))))
;(test '((lambda()1)))
;(run '((lambda()1)))

;(run '((lambda(a b)(set! a 1)) 1 2 ) )

(test '((lambda(a ) (set! b 1) ) 1))
